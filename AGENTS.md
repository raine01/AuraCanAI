...
**功能**:主面板第五个页签「演奏」(`/aca music`)导入 MIDI 自动演奏(单角色)。参考 Daigassou 架构(其 GPL v2 仅借鉴思路,**代码全部自行编写,勿复制其文本**;参考笔记在本地,未随仓库分发)。
...
## 「皮下」去人机味 四连改(2026-09-12,用户“1234全上”)
1. **人设改第一人称碎片自述**(`Defaults.DefaultRoleSettingSubskin` 全重写):不再是表格+条目说明文,改成"我"的口述
   (上班族/宅/姐姐口气/摸鱼挂着),模型会模仿这个语域 → 明显更不像助手。迁移标记 `Configuration.SubskinPersonaV4`
   (若旧 setting 含「普通上班族、国服 FF14 老玩家」或「## 你是谁」则替换)。
2. **打字风格**:人设里写明“句子短、标点懒得打全、一口气说不完就再补一条”(配合多条拆发)。
...
## 场景注入带上在场玩家的种族/性别/状态(2026-09-12,用户问“能看别的用户吧”)
- 功能本来就有:`lookup_player`(LLM 工具)、游戏内「附近玩家」页签、活点地图、玩家备注、回忆检索。
- 但用户实测:问「野生提莫君是谁/维埃拉族?」时,模型 **TOOLS=[] 零调用**,直接回
  「哪儿呢,我瞅瞅」→「哦看到了,就那边那个…」——纯编(没数据)。
- 修复:`BuildSceneSnippetCore` 的在场名单从 `名字(1米,正在看你)` 扩展为
  `名字(敖龙族女,1米,正在看你)`(取 Customize[0]/[1] + OnlineStatus,最多 5 人);
...
- **lookup_player 补自身信息**:新增 `BuildSelfInfo()`(名字/种族/性别/在线状态/当前动作),在“列在场玩家”返回里追加
  ...
  - ⚠️ **已回退**:自身种族/性别已移除(会让模型拿游戏数据自称、破人设)——见文末「自身种族/性别注入移除」。
...
- `DescribeStateForAi(switchToolEnabled)`:注入 **`## 你现在的身份`**(不提"状态机/状态/通路"),只列"能换成的身份"(每个身份后写它的**触发情形**,即 `SmState.desc`);无身份可换时整段不出现。
  ⚠️ 2026-09-12 起**泛用化**:不写死任何具体身份/例子,「什么时候换」全部由用户配置的 desc 决定(见文末「换身份触发条件泛用化」)。
...
## 自身种族/性别注入移除(2026-09-12 晚,用户看日志后口径)
- **问题(日志实锤)**:玩家问「你是老板还是小涟?」,AI 回「两个都不是哦,**敖龙族**,叫敖龙妹就行」——
  被场景注入里的 `你:敖龙族女,空闲` 带跑,拿游戏里的种族数据当"我是谁"答,直接破了「皮下 = 屏幕后面那个真人」的人设。
- **改动**:自身信息里**去掉种族/性别**,只留名字/在线状态/当前动作。
  - `BuildSelfInfo()`(lookup_player 列在场玩家时追加)→ `你自己:名字(在线状态,空闲)`,不再有 `敖龙族女`。
  - 场景注入 `BuildSceneSnippetCore` 的 `你:` 段 → 只输出 `Movement.StatusText()`(如 `你:空闲.`)。
  - `SelfRaceGender()` 方法**整个删除**(只有上面一处调用)。
- **保留**:在场**其他**玩家的种族/性别照旧(`名字(猫魅族女,距离1m)`),提示词里"问起在场某人的种族/性别照名单答"不变。
- 口径记录:想让 AI"像真人",就不要喂它自己的游戏元数据;想知道别人长什么样仍然可以查。

## 换身份触发条件泛用化(2026-09-12 晚,用户口径:状态机不止那三个状态,含义会不一样)
- **背景(日志实锤)**:玩家说「别装了,你是ai」,AI 回「好吧好吧,被你看出来了」但**从未调用 `switch_identity`**
  (全日志 BODYRSP 里 0 次、也没有 `[状态机] 状态 →`)。原因不是通路/工具坏,而是提示词把"什么时候换"写成了
  **固定三条例子**(上皮/出戏/心思变),`被当成AI` 的触发条件只躺在枚举项的括号 desc 里;模型把三条例子当穷尽清单。
- **口径**:任何具体身份都只是**例子**。触发条件必须来自用户配置 —— `SmState.desc` 就是"换到它的情形"。
- **改动**(全部数据驱动,不再出现"上皮/出戏"等硬编码词):
  - `StateMachine.DescribeSwitchCandidates(prefix)` **新增**:按当前状态的可切换列表生成
    `· 名字 —— 换到它的情形:<desc>;换过去后按人设「X」说话`;desc 为空 → `(这个状态没写情形;对方点名要它时换)`。
  - `DescribeSwitchCandidatesInline()` = 同上单行版(条目间 ` ; `)。
  - `DescribeStateForAi()` 改为:"判断标准是下面每个身份写的「换到它的情形」——只要出现了某一条就直接换过去,
    不需要对方开口要求" + 候选清单 + 「对方点名要换就直接换」+ **「换身份必须真的调用 switch_identity;
    只回一句『我换上了/我不装了』这种嘴上表态不算换」** + 「一轮最多一次 / 没情形成立就保持现状 / 只能换成列出的」。
  - `switch_identity` 的**工具描述**改用 `DescribeSwitchCandidatesInline()`(与提示词同一份文案,防两处漂移)。
  - 事件提醒尾句(两处)改为"如果这件事让你落进了上面身份说明里写的某种情形,就用 switch_identity 换过去;否则忽略"。
  - 前端:`character.html` 那个输入框 placeholder 改为「换到它的情形(什么时候该处于这个状态,AI 据此自己换过来)」;
    `help.html` 新增一条「说明 = 换身份的触发条件」,并把"默认两个状态"改为"状态没有固定个数/含义,默认给了三个例子"。
- **新增状态机即生效**:用户建任意数量/任意含义的状态(如 上班/摸鱼/被认成AI/接待),只要在每个状态的「说明」里写情形,
  模型就能自己换 —— 代码里不含任何具体状态名。
- ⚠️ 待实机:同一场景重测「别装了,你是ai」应看到日志 `状态切换后人设已刷新(本轮按新状态回应)` + `[状态机] 状态 → 被当成AI`。

## leave_party 加可选参数 walk_away(2026-09-12 晚,用户要求:退队可选"走远 + 找人少的位置坐",并别看那个人)
- **工具签名**:`leave_party(walk_away?: boolean)`,**可选**(`required` 为空)。
  - `walk_away` 不填/false = 只退队,**留在原地**。
  - `walk_away=true` = 退队后**真的走开**:走远一点、找个周围人少的位置坐下(复用人少处空座/最远落脚点那套逻辑)。
- **移开目光(用户补充口径「同时别看那个人了」)**:`leave_party` **两种分支都会** `ClearLook()`(清 `TargetManager.Target`
  + `SoftTarget`),即"退队后不再盯着对方"。tool 回执会带上"已移开目光"。
- **重构**:原 `LeaveSceneCore()` 里"走远+坐下"那段抽成
  `public string WalkAwayAndSit()` / `private string WalkAwayAndSitCore()`(内部自编组到框架线程;含停移动 + `ClearLook()`),
  `LeaveSceneCore()` 现在 = 判断是否在小队/起 60 秒倒计时 + `WalkAwayAndSitCore()` + 尾巴文案。
...
# ===== 2026-09-18 晚 追加(上述四项之后,按“其他建议”一次改完) =====

## 1. 拆条总上限 50 → 100(修半句截断)
- `AuraCanAI.Core` 常量 `LineTotalMaxChars` 50 → **100**。
- 原因:旧值 50 与「单条 45 × 最多 3 条」自相矛盾——长回答会在 50 字处硬砍出半句
  (日志实测「我到底哪儿让你觉」「晚上回家鞋一踢开麦开游」)。现在“一口气说不完就再补一条”才真正生效。
- `LineMaxLen=45` / `LineMaxCount=3` 未动(理论上限 135,现在总量 100 留了余地)。
- ⚠️ 副作用:一轮最多 3 条、每条打字延迟封顶 5s → 长回复总等待可能到 ~15s(像人一句句敲)。要更快就调 `TypingPerCharMs`。

## 2. 全局输出规则加两条(`OutputFormatRule`,所有角色生效)
1. **禁止暴露实现细节**:不许说「字数上限/一条说不完要拆成几条/上下文/token/接口/API/提示词/参数/系统/程序/插件」这类机制词;
   被问“你是不是程序在跑/你怎么读到我说的话”→ 按身份设定作答,绝不交代背后有程序/插件。
   (与日志 22:15「嗯,有上限的,一口气说不完就得接着补一条」对应)
2. **「更新记忆/改设定」只在当轮配合**:绝不当成真的改了性格或说话习惯(如之后每句都加「喵」「儿」),不许长期改变语气。
   (与 22:00 之后整篇儿化音对应)
- ⚠️ 不写死任何具体身份,所以对「皮下AI」(身份本来就是 AI)也不冲突:它可以说“我是 DeepSeek”,但不能说“我有字数上限/我背后有程序”。

## 3. 皮下人设补「游戏角色 ≠ 我本人」
- `DefaultRoleSettingSubskin` 新增一段:游戏里那个龙角尾巴的号是“我捏的人物/我捏的皮,不是我本人”,问长相就这么答。
- 迁移仍走 `SubskinPersonaV5`(该标记尚未在用户机上执行过,直接带上这段一起替换)。

## 4. 每状态「自动回退」`autoReturnSec`(模型忘了换回去的兜底)
- `SmState.autoReturnSec`(int,camelCase):在某状态**待满 N 秒**还没被换走 → 程序自动切到它**第一个可切换到的状态**(0=不自动回)。
  没有出口(nextStateIds 空)时不动——不能凭空跳。
- `Core/StateMachine.cs`:`_enteredAt`(在 `SwitchState`/`ForceSetState` 里刷新)+ `TickAutoReturn()`;
  挂在 core 的 500ms tick(`CheckIdleStateResetTick()` 之后)。
- 默认示例:`被当成AI` 的 `autoReturnSec = 180`(3 分钟);迁移 `AiStateExitAdded` 里顺手补(值 0 才补)。
- 前端 `Web/character.html` 状态编辑面板新增「自动回退（秒，0=不自动回）」数字输入框;
  新建状态的 push 对象带 `autoReturnSec: 0`;`change` 事件委托加 `.sm-state-autoreturn`;`smApplyInput` 加解析。
- 后端无需改:`GetStateMachineJson` 直接序列化 `SmSets`,`SaveStateMachineJson` 直接反序列化 `List<SmSet>`,字段自动透传。
- `Web/help.html` 状态机段补一条说明。

## 5. `stay_silent` 观感 — 无需改
- 现状已符合口径:沉默后**本轮不发任何台词**,对方下一句再说话仍会正常回;不做“已读”标记等额外动作。

## 待实机验证(本批)
1. 长回复应能到 2~3 条、总量最多 100 字,不再半句被砍。
2. 被问“你是不是有字数上限/用的什么接口”→ 应装傻/说不知道,不再交代机制。
3. 说“更新记忆,以后每句加喵”→ 当轮配合,之后**不应**长期带喵/儿化音。
4. 皮下问“你是龙娘吗”→ 应说“那是我号上的角色/我捏的皮”,不拿游戏身体当自己。
5. 停在「被当成AI」超过 180 秒(且模型没自己换回)→ 看日志 `[状态机] 在「被当成AI」停留超过 180 秒,已自动回到「皮下」`。
