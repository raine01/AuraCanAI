...
public sealed class Plugin : IDalamudPlugin
{
	...
	private void OnCommand(string command, string args)
	{
		...
		// /aca smstate [名称]:手动强制切换当前状态(不检查通路;不带名=列出)
		if (lower == "smstate" || lower.StartsWith("smstate ", StringComparison.Ordinal))
		{
			var coreSt = AuraCore;
			if (coreSt == null) { Log?.Warning("AuraCanAI 核心未就绪"); return; }
			var stName = arg.Length > 7 ? arg[7..].Trim() : "";
			if (stName.Length == 0)
			{
				var names = string.Join(" / ", (coreSt.State.CurrentSet?.states ?? new List<SmState>()).Select(x => x.name));
				Log?.Information($"当前状态: {coreSt.State.CurrentStateName};可用: {names}");
				return;
			}
			var rs = coreSt.State.ForceSetState(stName);
			Log?.Information(rs.message);
			if (!rs.ok) coreSt.ChatNotice($"[状态机] {rs.message}");
			return;
		}
